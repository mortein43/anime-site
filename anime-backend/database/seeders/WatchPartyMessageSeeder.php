...
class WatchPartyMessageSeeder extends Seeder
{
        ...
        for ($i = 0; $i < 50; $i++) {
            ...
            foreach ($viewers as $viewer) {
                ...
                if ($status !== WatchPartyStatus::WAITING) {
                    ...
                    $possibleSenders = collect([$user, $viewer]); // Host and viewer
                    foreach ($possibleSenders as $sender) {
                        for ($j = 0; $j < $messageCount; $j++) {
                            // Use joined_at for viewers, started_at for host
                            $messageTime = ($sender->id === $user->id) ? $party->started_at : $joinedAt;
                            if (!$messageTime) {
                                $messageTime = now()->subMinutes(rand(5, 120)); // Fallback
                            }
                            ...
                            WatchPartyMessage::create([
                                ...
                                'created_at' => (clone $messageTime)->addMinutes(rand(5, 60)),
                                'updated_at' => now(),
                            ]);
                        }
                    }
